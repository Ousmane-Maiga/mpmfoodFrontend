...
export default function LoginScreen() {
  ...
  const handleFormSubmit = async (
    ...
    try {
      ...
      await login({ employee_name: values.employee_name, pin: values.pin });

      // ✅ Auth context will now have the user, AppLayout will read it
      router.replace('/(tabs)');
    } catch (error: any) {
      console.error('Full error:', error);
      setSubmitting(false);
...
const styles = StyleSheet.create({
  ...
  connectButtonLabel: { color: '#000', fontSize: 18, fontWeight: 'bold' },
});
